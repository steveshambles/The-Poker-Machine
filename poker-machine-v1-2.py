...
V1.2 Update April 2020
----------------------
Got rid of annoying pop up msg box
that appears after every hand and that
you have to click. Now this only appears
on winning hands.

Fixed start new game bug, it didnt start a new game
it just sat there with all buttons disabled.

Added a new small window at bottom with some
bright and cheery graphics, totally unecessary.
Thanks to pixabay.com for images yet again.
"""
...
def logo_result_frame():
    ...
    PHOTO = PhotoImage(file=r'cards/result-frame.png')
    result_frame_lbl.config(image=PHOTO)
    result_frame_lbl.grid(row=6, column=0, padx=2, pady=2)
    result_frame_lbl.photo = PHOTO

# Need to call the  logo result frame def here.
logo_result_frame()

def show_winner_logo():
    """Display wining hand graphic."""
    result_frame_lbl = Label(result_frame)
    PHOTO = PhotoImage(file=r'cards/result-frame-winner.png')
    result_frame_lbl.config(image=PHOTO)
    result_frame_lbl.grid(row=6, column=0, padx=2, pady=2)
    result_frame_lbl.photo = PHOTO

def show_loser_logo():
    """Display losing hand graphic."""
    result_frame_lbl = Label(result_frame)
    PHOTO = PhotoImage(file=r'cards/result-frame-loser.png')
    result_frame_lbl.config(image=PHOTO)
    result_frame_lbl.grid(row=6, column=0, padx=2, pady=2)
    result_frame_lbl.photo = PHOTO
...
def winner():
    ...
    # Check there is enough credits in bank.
